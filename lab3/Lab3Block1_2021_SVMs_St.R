...
# err2, It has traied on both training ad validation error and is now tested, we can then assume that the model is atleast this good when also training on test data (filter3).
...
for(i in 1:10){ # We produce predictions for just the first 10 points in the dataset.
  ...
  for(j in 1:length(sv)){
    k2 <- c(k2, co[j] * kernal(as.vector(t(spam[sv[j],-58])), as.vector(t(spam[i,-58]))))
  }
  k<-c(k, sum(k2)+inte)
}
k
